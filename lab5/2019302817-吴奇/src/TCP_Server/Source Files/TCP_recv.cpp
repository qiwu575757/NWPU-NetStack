...
u_int8_t tcp_recv_buffer[1600] = { 0 };
...
int tcp_listen(TCPSocket* socketid, u_int8_t* buf, int buflen)
{
	...
	//tcb init
	tcb->server_initial_seq = 10000 * (rand() % 50);
	tcb->server_mss = MSS;
...
int tcp_accept1(TCPSocket* socketid, int acklen, int send_type)
{
	...
	tcp_pseheader* tcp_psehdr = (tcp_pseheader*)tcp_recv_buffer;
	int k;
	//对通信五元组的dest ip 和 dest port 进行更新
	for (k = 0; k < 4; k++)
		socketid->target_ip[k] = client_ip[k];
	socketid->target_port = client_port;
...
int tcp_accept2(TCPSocket* socketid, u_int8_t* buf, int buflen, int send_type)
{
	...
	printf("[TCP]   11the flag is %d", (int)tcp_hdr->flags);
	//receive third handmake
	if (tcp_hdr->flags  != ACK)//第三次握手第四次挥手的ack位为1
	{
...
int tcp_recvfrom(TCPSocket* socketid, u_int8_t* buf, int buflen)
{
	...
	printf("222the flag ==========%d   \n", (int)(tcp_hdr->flags));
	//receive data transfer
	if (((tcp_hdr->flags) != ACK) && ((tcp_hdr->flags) != FIN))
	{
	...
	if ((tcp_hdr->dest_port) != socketid->local_port) {
		printf("[TCP]Wrong Destination Port! %d-%d\n", ntohs(tcp_hdr->dest_port), socketid->local_port);
		return -1;
	}
	u_int16_t checknum = calculate_check_sum((ip_header*)tcp_recv_buffer, buflen + sizeof(tcp_psehdr));
	if (checknum != 0xffff && checknum != 0)
		...
		while (end == false)
		{
			...
			if (RECVBUFFER4.full == false)
			{
				...
				for (int i = 0; i < udpdata_len; i++)
				{
					...
					//printf("%c", RECVBUFFER4.pool[RECVBUFFER4.head][i]);
				}
				printf("\n");
...
DWORD WINAPI tcp_recv(LPVOID pM)
{
	...
	{
		//使用状态机对服务器端tcp接收进行控制
		//printf("[TCP] state machine is %d", server_recvstate);
		...
		case ESTABLISHED:
			...
			if (RECVBUFFER5.empty == false)
			{
				printf("size of packet is %d\n", RECVBUFFER5.size_of_packet[RECVBUFFER5.tail]);
			...
			break;
		case CLOSEWAIT:
			CLOSEWAIT_RESULT = tcp_accept1(socketid, 0, 2);//发送第三次挥手
