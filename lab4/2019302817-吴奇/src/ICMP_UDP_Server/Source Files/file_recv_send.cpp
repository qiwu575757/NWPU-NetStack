...
{
	FILE* fp = fopen("server_recv_data.txt", "w");
	printf("----------------------FILE RECEIVE---------------------------\N");
	while (recv_endflag == false || RECVBUFFER4.empty == false)
	{
